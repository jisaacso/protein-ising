function ProteinIsing3(A0,T,tf)
...
for(tStep=1:tf)
    ...
    if(ix>=3 && ix<=L-2 && iy>=3 && iy<=L-2)
        ...
        %now update J
        nhbrs = [j-L-1:j-L+1,j-1,j+1,j+L-1:j+L+1];
        idx=1;
        for(k=1:length(nhbrs))
            if(A2(nhbrs(k))==1 || A2(nhbrs(k))==-1)
                n2(idx) = nhbrs(k);
                idx = idx+1;
            end
        end
        %n2 = vector of indices in A corresponding to amino acids
        %neighboring j
        for(k=1:length(n2))
...
function e = h(a,J,nhbrsInA)
    L = 5;
    nhbrs = [2:4,7:9,12:14,17:19];
    e = 0;
    for(i = nhbrs)
%         J(nhbrsInA(i),nhbrsInA(i-1))
%         J(nhbrsInA(i),nhbrsInA(i+L))
%         J(nhbrsInA(i),nhbrsInA(i+L-1))
